...
const setupMicrophone = async () => {
  try {
    // Try to get microphone permission at the document level first
    await navigator.mediaDevices.getUserMedia({ audio: true });
    console.log("Microphone permission granted at document level");
  } catch (error) {
    console.error("Could not get microphone permission:", error);
    // Continue anyway - we'll handle the error in the iframe
  }
};

// Try to set up microphone access first
setupMicrophone().then(() => {
  console.log("Microphone setup complete, creating iframe");
  ...
  // Set allow attribute for microphone
